...
const RestroCard = (props) => {
    ...
    return (
        <div className="w-[250px] m-6 p-5 bg-gray-100 hover:bg-gray-300">
            ...
            <p className="text-1xl p-1">{name}</p>
            <p className="p-1">
                <span>{avgRating} stars</span>
                <span className="px-2">{sla.deliveryTime} mins</span>
            </p>
            <p className="p-1 text-[10px] text-black">{cuisines.join(",")}</p>
            </div>
        </div>
    )
}
export const withEnhancedRestroCard = (RestroCard) => {
    return (props) => {
        ...
        return (
            <>
                <p className="relative top-68 left-15 text-white">{aggregatedDiscountInfoV3.header} <span>{aggregatedDiscountInfoV3.subHeader}</span></p>
                <RestroCard {...props} />
            </>
        )
    }
